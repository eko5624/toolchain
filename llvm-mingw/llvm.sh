...
if [ -n "$HOST" ]; then
    ...
    *-mingw32)
        COMPILER_FLAGS=""
        COMPILER_FLAGS="$COMPILER_FLAGS -DCMAKE_SYSTEM_NAME=Windows"
        COMPILER_FLAGS="$COMPILER_FLAGS -DCMAKE_RC_COMPILER=x86_64-w64-mingw32-windres"
        COMPILER_FLAGS="$COMPILER_FLAGS -DCMAKE_ASM_COMPILER_TARGET=x86_64-w64-mingw32"
        COMPILER_FLAGS="$COMPILER_FLAGS -DCMAKE_C_COMPILER_TARGET=x86_64-w64-mingw32"
        COMPILER_FLAGS="$COMPILER_FLAGS -DCMAKE_CXX_COMPILER_TARGET=x86_64-w64-mingw32"
        ;;
    *)
